...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={`flex items-center gap-3 rounded-2xl px-4 py-3.5 font-sans text-sm font-medium transition-all duration-200 ${
        active
          ? "border border-brand-100/60 bg-[linear-gradient(110deg,#fff6fa,#f8e9ef)] text-brand-700 shadow-[inset_0_1px_0_rgba(255,255,255,.8)]"
          : "border border-transparent text-[#514c49] hover:bg-white/70 hover:text-foreground"
      }`}
    >
    ...
    <div className="relative min-h-screen overflow-hidden bg-transparent">
      ...
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-[272px] flex-col border-r border-[#e9e3de] bg-[linear-gradient(180deg,rgba(255,253,251,.97),rgba(252,248,245,.94))] px-5 py-7 lg:flex">
        ...
        <nav className="mt-12 space-y-2">
          <NavLink href="/" active={false} icon={Headphones}>Support Hub</NavLink>
          <NavLink href="/tickets" active={pathname.startsWith("/tickets")} icon={Ticket}>Tickets</NavLink>
          <NavLink href="/shared/tickets" active={pathname.startsWith("/shared")} icon={UsersRound}>Shared with me</NavLink>
          <NavLink href="/settings/connections" active={pathname.startsWith("/settings/connections")} icon={Link2}>Connections</NavLink>
          <NavLink href="/settings" active={pathname === "/settings" || pathname.startsWith("/settings/shares")} icon={Settings}>Organization settings</NavLink>
        </nav>
        {process.env.NEXT_PUBLIC_REVIEW_CONSOLE_URL ? (
          <div className="mt-8 border-t border-[#e9e3de] pt-7">
            <a
              href={process.env.NEXT_PUBLIC_REVIEW_CONSOLE_URL}
              className="group flex items-center justify-between px-2 font-sans text-sm font-medium text-brand-600 hover:text-brand-700"
            >
              Open Review &amp; Audit Console
        ...
        <div className="flex items-center gap-4">
          <details className="group relative lg:hidden">
            ...
            <nav className="argus-card absolute right-0 mt-3 w-64 rounded-2xl p-2">
              <NavLink href="/" active={pathname === "/"} icon={Headphones}>Support Hub</NavLink>
              <NavLink href="/tickets" active={pathname.startsWith("/tickets")} icon={Ticket}>Tickets</NavLink>
              <NavLink href="/shared/tickets" active={pathname.startsWith("/shared")} icon={UsersRound}>Shared with me</NavLink>
              <NavLink href="/settings/connections" active={pathname.startsWith("/settings/connections")} icon={Link2}>Connections</NavLink>
              <NavLink href="/settings" active={pathname === "/settings" || pathname.startsWith("/settings/shares")} icon={Settings}>Organization settings</NavLink>
      ...
      <main className="relative z-10 px-5 pb-12 pt-8 sm:px-8 lg:ml-[272px] lg:px-10 lg:pb-16 xl:px-14">
        <div className="mx-auto max-w-[1280px]">
          <div className="mb-7 flex justify-end">
            <OrgSwitcher className="argus-org-select max-w-[220px] rounded-xl border border-[#e6e0db] bg-white px-3.5 py-2.5 font-sans text-xs font-semibold text-foreground outline-none transition-colors hover:border-brand-200 focus:border-brand-400" />
          </div>
          {children}
        </div>
      </main>
